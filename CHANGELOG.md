...
- Projects & Engagement Knowledge Base (Phases B/C, following Phase A above): a `Project` entity replacing the ad hoc `client_case_id`, per-project role membership (`SystemAdmin`/`Consultant`/`ClientUser`), `require_admin`/`require_project_role` dependencies, a `Draft`→`Active` project lifecycle, and a per-project knowledge base for consultant-uploaded customer documents/audio/case studies, blended into retrieval alongside the existing Framework Knowledge Base. Design spec: `docs/superpowers/specs/2026-08-24-users-projects-engagement-kb-design.md`. Precedes and revises the items below.
