...
**Testing**: `pytest` + FastAPI `TestClient` (`httpx`) — a test bed exists (`tests/`, 316 tests, run via `pytest` from the repo root) covering the LLM Provider Abstraction modules, the chat-style interview modules (`chat_sessions`, `chat_engine`, `chat_endpoints`), the Phase A auth modules (`users_db`, `auth`, `/api/auth/*` endpoints), the Phase B project modules (`projects_db`, the new `auth` dependencies, `/api/projects/*` endpoints), the Phase C Engagement KB modules (`project_artifacts_db`, `project_knowledge_base`, `/api/projects/*/artifacts*` endpoints), the Backend API Integration modules (`process_db`, `responses_db`, `brief`, `RagEngine.search_merged`/`generate_comparative_benchmarks`, the new `/api/projects/*/evaluate`/`/responses`/`/brief` endpoints), and the Framework Authoring Mode modules (`framework_db`, the `/api/projects/*/framework/*` endpoints); the broader pre-migration API contract suite is still planned, not yet built (see Part 5). A GitHub Actions CI workflow (`.github/workflows/ci.yml`) runs the full suite on every push/PR to `main`.
...
└── tests/                     # pytest suite — 316 tests, see Part 6
```
...
A test bed now exists (`tests/`, 316 pytest tests, run via `pytest` from the repo root) covering essentially every module named above; see Part 6. The pre-migration API contract test suite once planned against `CASES_DATA`/`/api/evaluate` is moot — that API no longer exists.
...
**Tests**: a test bed exists now (`tests/`, 316 tests) covering the LLM Provider Abstraction modules, the chat-style interview modules (`chat_sessions`, `chat_engine`, `chat_endpoints`), the Phase A auth modules (`users_db`, `auth`, `/api/auth/*` endpoints), the Phase B project modules (`projects_db`, the new `auth` dependencies, `/api/projects/*` endpoints), the Phase C Engagement KB modules (`project_artifacts_db`, `project_knowledge_base`, `/api/projects/*/artifacts*` endpoints), the Backend API Integration modules (`process_db`, `responses_db`, `brief`, `RagEngine.search_merged`/`generate_comparative_benchmarks`, the new `/evaluate`/`/responses`/`/brief` endpoints), and the Framework Authoring Mode modules (`framework_db`, `/api/projects/*/framework/*` endpoints) — run `pytest` from the repo root. A previously-tracked "broader pre-migration API contract test bed" (case-study endpoints, `/api/evaluate`'s response shape) is no longer a planned item — that API was retired 2026-08-31, so there's nothing left to write contract tests against; `documentation/testing/test-strategy.md` still describes it and needs updating.
