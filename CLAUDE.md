...
**Testing**: `pytest` + FastAPI `TestClient` (`httpx`) — a test bed exists (`tests/`, 125 tests, run via `pytest` from the repo root) covering the LLM Provider Abstraction modules, the chat-style interview modules (`chat_sessions`, `chat_engine`, `chat_endpoints`), the Phase A auth modules (`users_db`, `auth`, `/api/auth/*` endpoints), and the Phase B project modules (`projects_db`, the new `auth` dependencies, `/api/projects/*` endpoints); the broader pre-migration API contract suite is still planned, not yet built (see Part 5).
...
The chat session endpoints are additive — `/api/evaluate` is unchanged and still used by the current form-based frontend. See `docs/superpowers/specs/2026-08-26-chat-style-interview-design.md`. The auth endpoints (Phase A) and project endpoints (Phase B, both 2026-08-28) are likewise additive — no pre-existing endpoint requires a token or project membership yet; see `docs/superpowers/plans/2026-08-28-phase-a-users-auth.md` and `docs/superpowers/plans/2026-08-28-phase-b-projects.md`. `POST /api/projects/{id}/members` does not exist yet — only the initial Consultant, assigned atomically at creation, can be added through the API today; there is no way to add a `ClientUser` to a project via the API yet.
...
**Also planned (not yet implemented)**: `project_artifacts` table (incl. `purpose` tagging), plus a `pgvector` table (`project_kb_chunks`) for the Engagement Knowledge Base (Phase C). `POST/GET/DELETE /api/projects/{id}/artifacts`. `responses.client_case_id` becomes `responses.project_id` — the `responses` table itself already exists (Phase B) but is DDL-only, nothing reads/writes it yet. `GET /api/cases` and `GET /api/case/{case_id}` are planned for removal, replaced by the `/api/projects` endpoints, once Backend API Integration cuts the frontend over. Full detail: [Technical Spec §3-4](documentation/development/technical-spec.md).

---

# Part 5: Current Status & Roadmap

**As of 2026-08-28: Phase 0 (Database Platform), Phase A (Users & Auth), and Phase B (Projects) are done; Phase C (Engagement KB) has not started.** The BRD, functional spec, technical spec, and architecture docs all describe the target Framework Factory design; `backend/main.py` still runs the old hardcoded Blazar/Basil case-study critic alongside the new auth/project endpoints — nothing in the evaluation flow is project-scoped yet. `backend/database.py` and `backend/rag_engine.py` have been migrated off SQLite + flat-file storage onto Neon Postgres + `pgvector` (Phase 0) — see the Foundational Work section of the Roadmap.
...
**Tests**: a test bed exists now (`tests/`, 125 tests) covering the LLM Provider Abstraction modules, the chat-style interview modules (`chat_sessions`, `chat_engine`, `chat_endpoints`), the Phase A auth modules (`users_db`, `auth`, `/api/auth/*` endpoints), and the Phase B project modules (`projects_db`, the new `auth` dependencies, `/api/projects/*` endpoints) — run `pytest` from the repo root. The broader pre-migration API contract test bed (case-study endpoints etc.) described in [Test Strategy](documentation/testing/test-strategy.md) is still not built and still pending explicit go-ahead; once it lands, its tests will be pinned to the pre-migration API contract and will need rewriting once the roadmap's API overhaul lands.
