...
- Audio is transcribed automatically (Google Speech-to-Text); if that's unavailable, the artifact is marked `Transcript Needed` rather than failing outright — a Consultant can then paste the transcript by hand via `POST /api/projects/{id}/artifacts/{artifact_id}/transcript` to complete that loop.
