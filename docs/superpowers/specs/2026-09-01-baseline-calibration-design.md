...
No changes to `chat_sessions`/`chat_messages` schema — `current_level_index` (already a generic integer position tracker) is reused to track calibration-concept index while `phase` is `'calibration_awaiting_answer'`; two new `message_type` values (`calibration_prompt`, `calibration_feedback`) are added to the existing free-text column, not a new schema element.
