...
- **No `RESEND_API_KEY` — tracked as a follow-up, not immediate.** The Consultant-Initiated Client Invites feature already degrades gracefully to a copy/paste setup link without it. Revisit when there's a Resend API key to provision; two things block wiring it up as-is: (1) `backend/email_provider.py`'s `FROM_ADDRESS` is hardcoded to `onboarding@cosmos-strategy.example` — a non-functional placeholder domain (`.example` is a reserved TLD) that Resend will reject sends from, so this needs either a real Resend-verified sending domain or switching to Resend's built-in test sender (`onboarding@resend.dev`, works immediately but Resend restricts test-mode sends to the account's own email until a custom domain is verified); (2) `FRONTEND_BASE_URL` is also unset — trivial to set now (this deployment is single-service, so it's just the `cosmos-dev` service's own URL), worth doing in the same pass as the Resend key.
