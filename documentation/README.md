...
- **Phase**: Insights POC — feature-complete in code through Known Gaps Cleanup (2026-09-17): Framework Authoring Mode, the async artifact-ingestion pipeline, voice input on the chat interview, the Chat Interview Screen UI/UX Redesign, the App-Wide Light Professional Retheme, and Send Engagement Report are all built. A GCP dev deployment is specced (2026-09-17/18, superseding the earlier 2026-08-25 production plan) but not yet executed — nothing has ever been applied to a live GCP project.
...
- **Test bed**: `pytest`, 484 tests, run via CI on every push/PR to `main` — see [Test Strategy](./testing/test-strategy.md).
