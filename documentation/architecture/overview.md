...
- **Framework Knowledge Base** (Section 1; **done for the seeded content, upload mechanism not built**): the shared Cosmos methodology materials — migrated from the Brand Compass decks in `archives/` → `data/vector_db.json` (flat file) into the `framework_kb_chunks` table (`pgvector`, HNSW-indexed). One global index, common to every project. Today it's populated **only** by `rag_engine.py`'s startup ingestion of the two bundled `archives/*.pdf` files — there's no admin-facing way to add to it. A **Cosmos Knowledge Uploads** capability (new requirement, 2026-09-05, not designed) would let Cosmos widen this shared index directly, which is also the concrete mechanism DIY self-serve mode (Engagement Delivery Mode (a)) needs in order to frame questions without a Consultant curating content. See `documentation/product/roadmap.md`'s "Cosmos-Owned Content Repositories" section.
...
## 4. Guided Learning Flow (Designed; Two of Nine Items Built)
...
- **Only half of the case-study resolution mechanism exists.** `purpose`-tagged artifact exclusion (Section 3.1) is real and implemented — `RagEngine.search_merged` filters `purpose='case_study_resolution'` chunks out of normal retrieval, so the AI can't leak the hidden answer during ordinary Q&A — and the frontend/backend both accept `case_study_external`/`case_study_internal`/`case_study_resolution` as artifact purposes. But there is no actual "reveal" step: `chat_engine.py` has no case-study-specific phase, no endpoint presents a case study or its resolution, and no seeded provocations exist. A tagged case-study artifact today just sits in the Engagement KB like any other document, contributing to ordinary benchmark generation minus the resolution content — confirmed by grep, not just absence from the roadmap checklist.
...
- The 2026-09-11 planning session (moved from an original 2026-09-09 date) resolved the open question about baseline calibration's position: it stays pre-question, as built — though it should carry Cosmos's own prescribed concept definitions rather than an org-customized framing, which is not yet built. See `documentation/product/stakeholder-clarifications-2026-09.md`.
