...
## Current Phase: Insights POC — Foundational Work (Phases 0/A/B/C) Complete, Migration Checklist Not Started
...
**Phases 0, A, B, and C (Database Platform, Users & Auth, Projects, Engagement Knowledge Base) are all complete — see the Foundational Work section below.** Backend API Integration and Frontend GUI Overhaul have not been implemented yet. The running code (`backend/main.py`) still reflects the old design for its core evaluation flow: an in-memory `CASES_DATA` dict with the Blazar/Basil cases still serves `/api/evaluate` and the current frontend, and the `responses` table (added in Phase B) is DDL-only — nothing reads or writes it yet. `backend/database.py` and `backend/rag_engine.py` run on Neon Postgres + `pgvector` rather than SQLite + a flat file, real login/registration and project provisioning exist via `/api/auth/*` and `/api/projects/*`, and a Consultant can now upload documents/audio into a project's Engagement Knowledge Base via `/api/projects/{id}/artifacts` — but nothing in the current evaluation flow requires, queries, or blends any of this yet; `/api/evaluate` still only searches the Framework Knowledge Base.
...
This item depended on Phase A/B/C being built first (auth + the `project_id` model + a queryable Engagement KB) — all three are now done (2026-08-28), so this item is fully unblocked.
