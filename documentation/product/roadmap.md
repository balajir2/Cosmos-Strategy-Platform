...
## Current Phase: Insights POC — Specs Complete, Implementation Not Started (except Phase 0)
...
**Phase 0 (Database Platform) is complete — see the Foundational Work section below.** Everything else in the migration checklist below (Phases A/B/C, Backend API Integration, Frontend GUI Overhaul) has not been implemented yet. The running code (`backend/main.py`) still reflects the old design: an in-memory `CASES_DATA` dict with the Blazar/Basil cases, no `responses` table of any kind (legacy or target-shaped — see the Foundational Work section), and no login of any kind. `backend/database.py` and `backend/rag_engine.py`, however, now run on Neon Postgres + `pgvector` rather than SQLite + a flat file.
