...
## Current Phase: Insights POC — Foundational Work Phases 0/A/B Complete, C and Migration Checklist Not Started
...
**Phases 0, A, and B (Database Platform, Users & Auth, Projects) are complete — see the Foundational Work section below.** Phase C (Engagement Knowledge Base), Backend API Integration, and Frontend GUI Overhaul have not been implemented yet. The running code (`backend/main.py`) still reflects the old design for its core evaluation flow: an in-memory `CASES_DATA` dict with the Blazar/Basil cases still serves `/api/evaluate` and the current frontend, and the `responses` table (added in Phase B) is DDL-only — nothing reads or writes it yet. `backend/database.py` and `backend/rag_engine.py` run on Neon Postgres + `pgvector` rather than SQLite + a flat file, and real login/registration and project provisioning now exist via `/api/auth/*` and `/api/projects/*`, but nothing in the current evaluation flow requires or uses them yet.
...
This item depended on Phase A/B being built first (auth + the `project_id` model it requires) — both are now done (2026-08-28), so this is unblocked. Still depends on Phase C for the merged Engagement KB retrieval bullet above.
