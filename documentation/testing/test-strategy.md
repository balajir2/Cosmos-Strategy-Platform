...
- **Scope**: 491 tests covering essentially every backend module — see "What's Covered" below.
...
| Send Engagement Report | `test_responses_db.py` (`benchmark_level_1/2/3` persistence/COALESCE), the benchmark-JSON-parsing cases in `test_chat_engine.py` (valid JSON and the legacy-plain-text fallback), `test_brief.py` (`compile_brief_html`, incl. an HTML-escaping/XSS case), `test_email_provider.py` (`send_report_email`), `test_send_report_endpoint.py` — the frontend buttons (chat completion screen, Consultant project setup page) have no automated coverage, per "Not covered" below |
